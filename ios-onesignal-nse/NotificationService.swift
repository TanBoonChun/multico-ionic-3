...
// REQUIREMENT: This extension must share an App Group with the main app target
...
//
// SDK NOTE: the app is on onesignal-cordova-plugin 2.11.2 -> OneSignal iOS SDK 2.16.1.
// In SDK 2.x the module is `OneSignal` (there is no `OneSignalExtension`; that only
// exists in SDK 3/5), and the extension helpers RETURN the content instead of taking
// a completion handler, so we call contentHandler ourselves.
