...
/**
 * Generated class for the GensetsparepartPage page.
 *
...
@IonicPage()
@Component({
  selector: 'page-gensetsparepart',
  templateUrl: 'gensetsparepart.html',
})

export class GensetsparepartPage {
  ...
  request() {
    this.storage.get('token').then(val => {
      this.http.post(SERVER_URL + '/serviceticket/pendingSparepart?token=' + val.token, {item:this.arr,req:this.reqId,serId:this.serId }, {})
        .subscribe(result => {
          {
            this.navCtrl.pop();
            console.log(result);
