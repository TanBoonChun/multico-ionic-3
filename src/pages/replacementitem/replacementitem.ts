...
@IonicPage()
@Component({
  selector: "page-replacementitem",
  templateUrl: "replacementitem.html",
})
export class ReplacementitemPage {
  private Id: any;
  ...
  loadData() {
    ...
    this.storage.get("token").then((val) => {
      data = this.http.get(
        SERVER_URL + "/techbag?token=" + val.token
      );
  ...
  addReplacement() {
    this.hide = false;
    let modal = this.modalCtrl.create("ReplacementitemmodalPage", {
      x: this.x,
      genset: this.genset_qr,
    });
    modal.present();
  ...
  submit() {
    ...
    if (this.modalData.length != 0) {
      this.storage.get("token").then((val) => {
        return this.http
          .post(
            SERVER_URL + "/replacement?token=" + val.token,
            { data: this.modalData, id: this.Id, type: "Replacement" },
            {}
          )
          .subscribe((result) => {
            if (result > 0) {
              this.toast
                .show("Success", "7000", "center")
                .subscribe((toast) => {});
              this.navCtrl.pop();
            }
            (err) => {
  ...
  getList0(ev: any) {
    ...
    if (serVal && serVal.trim() != "") {
      this.items = this.items.filter((item) => {
        return (
          (item.status &&
            item.status.toLowerCase().indexOf(serVal.toLowerCase()) > -1) ||
          // || item.acceptedName.toLowerCase().indexOf(serVal.toLowerCase()) > -1
          // || item.payment_terms.toLowerCase().indexOf(serVal.toLowerCase()) > -1
          (item.Item_Code &&
            item.Item_Code.toLowerCase().indexOf(serVal.toLowerCase()) > -1) ||
          (item.Part_Name &&
            item.Part_Name.toLowerCase().indexOf(serVal.toLowerCase()) > -1) ||
          (item.Serial_No &&
            item.Serial_No.toLowerCase().indexOf(serVal.toLowerCase()) > -1)
        );
      });
    } else {
      this.onCancel(ev);
